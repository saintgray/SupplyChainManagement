...
pForm.gfnTransaction = function(strSvcId, strSvcUrl, inData, outData, strArg, callBackFnc, isAsync)
{
	...
	var strServiceUrl = nexacro.getEnvironmentVariable("gvServerUrl") + strSvcUrl;
	// 5. strArg
	var arguments = "";
...
/**
 * @공통 Combo 코드 조회 함수 <br>
 * 넘어온 공통 그룹코드로 상세코드, 명을 넘어온 DataData 에 채운다. 
 * @param {String} grpCd - 그룹코드
 * @param {Dataset} 결과 채울 Dataset
 * @return N/A
 */
//pForm.gfnCommCdCombo = function(grpCd,outDs)
pForm.gfnCommCdCombo = function(grpCd,comboName)
{
    var objDs = new Dataset();
	
    objDs.name = "dsMain"+comboName.name;
	
	//trace("gfnCommCdCombo!!!!!!!!!!!!!!!!!!!!!!!!!!  :  " + objDs.name);
    ...
    this.addChild("dsMain"+comboName.name, objDs);	
	
    //if(this.gfnIsNull(grpCd)) {
    //   outDs.deleteRow();
	//   return false;
    //}	

	var strSvcId    = "selectcommcdcombo";
	var strSvcUrl   = "selectcommcdcombo.do";
	var inData      = "";
	var outData     = "dsMain"+comboName.name+"=dsList";
	var strArg      = "grpCd="+grpCd;
	var callBackFnc = "";
	...
	this.gfncommonTransaction( strSvcId , 	// transaction을 구분하기 위한 svc id값
	...
	//trace("this.dsMain.rowcount : " + this.dsMain.rowcount);							
	//trace("dd     "+outDs);
	
	//trace("this.dsMain "+this.dsMain.saveXML());
/*	
	for (var i=0; i< this.dsMain.rowcount; i++)
	{
		var nRow = outDs.addRow();
		outDs.copyRow(nRow, this.dsMain, i);
	}  	
*/

   comboName.set_innerdataset("dsMain"+comboName.name);
   comboName.set_codecolumn("code");
   comboName.set_datacolumn("name");
   
	//trace("comboName.getCount() : "+comboName.getCount());
};
 ...
 * this.gfnTransaction("save", strSvcUrl, inData, outData, strArg, "fnCallback", true);
 */ 
pForm.gfncommonTransaction = function(strSvcId, strSvcUrl, inData, outData, strArg, callBackFnc, isAsync)
{
    //trace("gfncommonTransaction !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
