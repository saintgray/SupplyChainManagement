...
function comcombo(group_code, combo_name, type, selvalue, url){
	...
	$.ajax({ 
	     type: "POST",  
	     url: (url==null || url==undefined)?"/commonproc/comcombo.do":url, 
	     dataType: "json",  
	     data : data,
	     success: function(data)
	     { 				
...
//selectComCombo(comtype, combo_name, type, searchkey,selvalue)	
//모델콤보일경우      type : m    pdcode : 장비코드    pdmdno : null
//제품콤보일경우      type : s    pdcode : 장비코드    pdmdno : 모델번호 
//combo_name   select id
//type    sel : 선택   all : 전체

function selectGoodComCombo(dtype, pdcode, pdmdno, combo_name, type, selvalue){
